...
export class WorkbenchPlugin extends SystemPlugin {
    ...
    protected async onLoad(): Promise<void> {
        ...
        this.app.api.invoke('controller:register', 'settings-button', SettingsButton);
        this.app.api.invoke('zone:register', 'ribbon-bottom', { component: 'settings-button', priority: 0 });

        // Auto-Login Check
        try {
            // Check if config-manager is available and get last opened vault
            const lastOpened = await this.app.api.invoke('config:get', 'vault.last-opened') as string;
            if (lastOpened && lastOpened.trim() !== '') {
                ...
                this.app.events.emit('app:vault-opened', { path: lastOpened });
            }
        } catch (error) {
