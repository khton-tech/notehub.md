...
const APPS: AppConfig[] = [
    {
        ...
        generatedDir: path.join(ROOT_DIR, 'apps', 'desktop', 'src', 'generated'),
    },
    {
        name: 'capacitor',
        dir: path.join(ROOT_DIR, 'apps', 'capacitor'),
        generatedDir: path.join(ROOT_DIR, 'apps', 'capacitor', 'src', 'generated'),
    },
];

const GENERATED_DIRS = APPS.map(a => a.generatedDir);
...
/**
 * Read an app's package.json dependencies to determine available plugins
 */
function getAppPluginPackages(app: AppConfig): Set<string> {
    ...
    try {
        ...
        return new Set(Object.keys(deps).filter(d => d.startsWith('@notehub/')));
    } catch {
        return new Set();
    }
}
...
/**
 * Generate plugin-imports.ts for each app.
 * The file contains a switch-based importPlugin function that Vite can
 * statically analyze for code-splitting.
 */
function generatePluginImports(manifests: PluginManifest[]): void {
    for (const app of APPS) {
        const appPackages = getAppPluginPackages(app);

        // Filter manifests to only plugins this app has as dependencies
        const appManifests = manifests.filter(m => {
            const pkg = pluginIdToPackageName(m.id);
            return appPackages.has(pkg);
        });

        // Group by type for readability
        const system = appManifests.filter(m => m.type === 'system');
        const ui = appManifests.filter(m => m.type === 'ui');
        const feature = appManifests.filter(m => m.type === 'feature' || m.type === 'features');

        const cases: string[] = [];

        const addGroup = (label: string, plugins: PluginManifest[]) => {
            if (plugins.length === 0) return;
            cases.push(`        // ${label} plugins`);
            for (const m of plugins) {
                const pkg = pluginIdToPackageName(m.id);
                cases.push(`        case '${pkg}':`);
                cases.push(`            return import('${pkg}');`);
            }
        };

        addGroup('System', system);
        addGroup('UI', ui);
        addGroup('Feature', feature);
...
`;
        ...
        fs.mkdirSync(app.generatedDir, { recursive: true });
        const outFile = path.join(app.generatedDir, 'plugin-imports.ts');
        fs.writeFileSync(outFile, content, 'utf-8');
        console.log(`   ✅ ${path.relative(ROOT_DIR, outFile)} (${appManifests.length} plugins)`);
    }
}
...
async function main(): Promise<void> {
    ...
    // Step 3: Generate plugin import maps
    console.log('\n📦 Generating plugin import maps...');
    generatePluginImports(manifests);
